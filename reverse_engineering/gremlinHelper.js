...
const initializeSshTunnel = async (info, sshService) => {
	const sshConnectionConfig = getSshConfig(info);
...
const connect = async (info, logger, sshService) => {
	if (info.ssh) {
		info = await initializeSshTunnel(info, sshService);
		sshTunnel = true;
